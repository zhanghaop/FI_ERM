...
public class DimensTable extends BatchBillTable implements IComponentWithActions {
	...
	public void initUI() {
		super.initUI();
		//参照选择下拉框编辑器
		getBillCardPanel().getBodyPanel().setTableCellEditor(ReimRuleDimVO.REFERENTIAL,
				new BillCellEditor(getDtRefPane()));
	}
	
	public ReimRuleDimVO[] checkReimDims(ReimRuleDimVO[] reimDimVOs,String pk_billtype,String pk_group,String pk_org)
	...
	public List<SuperVO> Save() throws Exception{
		...
		List<ReimRuleDimVO> returnVos = NCLocator.getInstance().lookup(
				IReimTypeService.class).saveReimDim(str[0], getModel().getContext().getPk_group(),str[1],
						returnvo);
