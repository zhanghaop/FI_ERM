	...
	public void doAction(EditorContext context) throws BusinessException {
		...
		case TYPE_NEW:
			...
			if (context.getTranstype() != null) {
				getcardpanel().setBillValueVO(
						converttoBilltype(context.getTranstype()));
			} else {
...
//		if (BXUiUtil.getDefaultOrgUnit().equalsIgnoreCase(BXConstans.GROUP_CODE)) {
//			
//			return new ButtonObject[] {((DefaultListenerController) getDjlxpanel().getListenerController()).getBtnMap().get(
//			"sysinit_distribute")};
//		}
	...
	public Object getTransTypeExtObj(EditorContext context)
		...
		case TYPE_NEW:
			vo = queryvoBycontext(context);
			if (vo != null) {
				getcardpanel().getBillCardPanelDj().getHeadItem("djlxoid")
						.setValue(vo.getDjlxoid());
			}
			try {
				getcardpanel().getBillCardPanelDj().getHeadItem("djlxbm")
						.setValue(transtype.getPk_billtypecode());
				getcardpanel().getBillCardPanelDj().getHeadItem("djlxmc")
						.setValue(transtype.getBilltypename());
				getcardpanel().getBillCardPanelDj().getHeadItem("djlxjc")
						.setValue(transtype.getBilltypename());
				getcardpanel().getBillCardPanelDj()
						.getHeadItem("djlxmc_remark").setValue(
								transtype.getBilltypename());
				getcardpanel().getBillCardPanelDj()
						.getHeadItem("djlxmc_remark").setValue(
								transtype.getBilltypename());
//				getcardpanel().getBillCardPanelDj()
...
//			head.setPk_group("@@@@");
	...
	private BillTypeVO queryvoBycontext(EditorContext context) {
		try {
			BillTypeVO[] vos = getdjlxiterface().queryBillTypeByBillTypeCode(
					context.getTranstype().getPk_billtypecode(),
...
//			template = getDjlxpanel().getDjlxCardPanel();// new
			...
			inittemplate();
			getcardpanel().getBillCardPanelDj().setBillData(
					getcardpanel().getBillCardPanelDj().getBillData());
		}
		return template;
	}
	protected void inittemplate() {
		...
		try {
			getcardpanel().getBillCardPanelDj().getHeadItem("djlxmc").setShow(
					false);
			getcardpanel().getBillCardPanelDj().getHeadItem("djlxjc").setShow(
					false);
			getcardpanel().getBillCardPanelDj().getHeadItem("djlxbm").setShow(
					false);
		} catch (Exception e) {
		}
	}
...
//		head.setDwbm(BXConstans.GROUP_CODE);
	...
	protected String getDjlxPanelClassName() throws InstantiationException,
