package nc.ui.erm.matterapp.bugdgetinit;
 ...
 *
 */
public class ErmMatterAppPfClientBizProcess implements IPFClientBizProcess {
		...
		for (Iterator iterator = argsList.iterator(); iterator.hasNext();) {
			...
			if (budgetControl_id.equals(df.getName())) {//预算控制
				if (UFBoolean.valueOf(value.toString()).booleanValue()) {
					...
					try {
						tpcontrolvo = ErmMtAppFlowCheckInfo.doNtbCheck((AggMatterAppVO)billvo);
