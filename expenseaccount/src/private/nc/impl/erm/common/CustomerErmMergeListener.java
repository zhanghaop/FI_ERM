...
/**
 * <p>
 * 报销管理客户合并监听类 需要采用事件注册机制，注册方式通过系统 业务插件注册-动态企业建模平台-客户基本信息-合并前
 * 此处需要注意：(1)注册对应的监听类，并且注意插件执行顺序号，此处是按照需要编码进行执行的，
 * 也就是说，当跟其他产品安装的时候注意编排需要，沟通uap实现。
 * </p>
 * 
 * 修改记录：<br>
 * <li>修改人：chenshuaia 修改日期：2013-04-07 修改内容：增加申请单、结转单、摊销信息的修改，并将业务代码提出</li> <br>
 * <br>
 * 
 * @see
 * @author liansg
...
public class CustomerErmMergeListener implements IBusinessListener {
	...
	public void doAction(IBusinessEvent event) throws BusinessException {
		...
		try {
			NCLocator.getInstance().lookup(IErmMergeService.class).mergeCustomer(targetCus, sourceCus);
