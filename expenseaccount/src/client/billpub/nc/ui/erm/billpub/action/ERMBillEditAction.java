...
public class ERMBillEditAction extends EditAction{
    ...
    public boolean check() throws BusinessException {
		...
		if(selectedOperaDatas != null){
			if(selectedOperaDatas.length!=1){
				throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl
						.getNCLangRes().getStrByID("2011", "UPP2011-000913")/** @res* "请选择一张单据进行修改"*/);
			}
		}
		JKBXVO jkbxvo = (JKBXVO) getModel().getSelectedData();
		if(jkbxvo==null) return false;
		
		if (jkbxvo.getParentVO().getQcbz().booleanValue()) {//期初单据
			msg = ActionUtils.checkBillStatus(jkbxvo
					.getParentVO().getDjzt(), ActionUtils.EDIT, new int[] {
					BXStatusConst.DJZT_Saved, BXStatusConst.DJZT_TempSaved,
					BXStatusConst.DJZT_Sign });
			if (jkbxvo.getContrastVO() != null
		...
		if (!jkbxvo.getParentVO().getQcbz().booleanValue()) {
			// 设置授权代理人
			HeadFieldHandleUtil.initSqdlr(getEditor(), getEditor().getBillCardPanel()
					.getHeadItem(JKBXHeaderVO.JKBXR),
					((ErmBillBillManageModel) getEditor().getModel())
							.getCurrentBillTypeCode(), getEditor()
	...
	protected boolean isActionEnable() {
		...
		if (!BXConstans.BXLR_QCCODE.equals(nodeCode)) {
			return getModel().getUiState() == UIState.NOT_EDIT && selectedData != null && ((JKBXVO) selectedData).getParentVO().getSpzt().intValue()!=IPfRetCheckInfo.NOPASS
