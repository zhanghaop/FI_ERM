...
public class ContrastAction extends NCAction {
	...
	public static void doContrastToUI(BillCardPanel card, JKBXVO vo, List<BxcontrastVO> contrastsData,
		...
		String[] changeHeadFields = new String[] { JKBXHeaderVO.ZFYBJE, JKBXHeaderVO.ZFBBJE, JKBXHeaderVO.GROUPZFBBJE,
		...
		if (contrastsData == null || contrastsData.size() == 0) {
			...
			for (BillTabVO billTabVO : billTabVOs) {
				...
				if (card.getBillModel(tabcode) != null) {
					...
					for (int i = 0; i < rowCount; i++) {
						...
						card.setBodyValueAt(card.getBillModel(tabcode).getValueAt(i, JKBXHeaderVO.YBJE), i,
								JKBXHeaderVO.ZFYBJE, tabcode);
						card.setBodyValueAt(card.getBillModel(tabcode).getValueAt(i, JKBXHeaderVO.BBJE), i,
								JKBXHeaderVO.ZFBBJE, tabcode);
						card.setBodyValueAt(card.getBillModel(tabcode).getValueAt(i, JKBXHeaderVO.GROUPBBJE), i,
								JKBXHeaderVO.GROUPZFBBJE, tabcode);
						card.setBodyValueAt(card.getBillModel(tabcode).getValueAt(i, JKBXHeaderVO.GLOBALBBJE), i,
								JKBXHeaderVO.GLOBALZFBBJE, tabcode);
			...
			if (billModel != null) {
				...
				if (BXConstans.BILLTYPECODE_RETURNBILL.equals(card.getHeadItem(JKBXHeaderVO.DJLXBM).getValueObject()
						.toString())) {
	 ...
	 */
	private static void setValueToEditor(BillCardPanel card, JKBXVO vo, List<BxcontrastVO> contrastsData,
			String[] headFields, String[] changeHeadFields, BillModel billModel) throws BusinessException {
		// 设置主表，业务行，冲销行；业务行，冲销行包含新增，修改和删除的行
		// 主表
		...
		// 分页签设置业务行
		Map<String, List<BXBusItemVO>> tableCode2VOMap = VOUtils.changeCollection2MapList(
				Arrays.asList(vo.getChildrenVO()), new String[] { BXBusItemVO.TABLECODE });
