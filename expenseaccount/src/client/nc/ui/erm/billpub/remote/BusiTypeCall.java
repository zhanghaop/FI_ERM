...
public class BusiTypeCall extends AbstractCall implements IRemoteCallItem {
	private BillForm panel = null;

	public BusiTypeCall(BillForm panel) {
		this.panel = panel;
	}
	...
	public ServiceVO getServcallVO() {
		...
		callvo.setParam(new Object[] { BusiTypeVO.key,billTypeCodes});
		return callvo;
	}
	...
	public void handleResult(Map<String, Object> datas)
		...
		Map<String, IConfigVO> vos = (Map<String, IConfigVO>) datas.get(callvo
				.getCode());
		if (vos != null) {
			for (String key : vos.keySet()) {
				IConfigVO configVO = vos.get(key);
				((ConfigAgent) ConfigAgent.getInstance()).getCache()
						.putCommonVO(BusiTypeVO.key, configVO);
			}
		}
	}
