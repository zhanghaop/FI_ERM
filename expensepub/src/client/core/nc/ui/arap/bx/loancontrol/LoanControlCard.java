...
//			ivjUIcheckBB.setText(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011","UPP2011-000034")/*@res "按本位币控制"*/);
	...
	private void setAVO(SuperVO vo) {
		...
		getRefbzbm().setPKs(new String[]{controlVO.getCurrency()});
	...
	public SuperVO getVO() throws BusinessException{
		...
		Collection<SuperVO> vos = impl.getVOs(parentUI.getVoClass(), parentUI.getWhereStr(), true);
	    ...
	    for(Object svo:vos){
			...
			if((name == null || "".equals(name)) && (code1 == null || "".equals(code1))){
				throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("expensepub_0","02011002-0024")/*@res "需要录入名称和编码"*/);
			}
			if(name == null || "".equals(name)){
				throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("expensepub_0","02011002-0025")/*@res "需要录入名称"*/);
			}
			if(code1 == null || "".equals(code1)){
				throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("expensepub_0","02011002-0026")/*@res "需要录入编码"*/);
			}
...
//--end	modified by Chen deyin(chendya@ufida.com.cn)
        ...
        if(!isGroup()){
        	if(pk_org==null||pk_org.length()==0){
    			throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("expensepub_0","02011002-0029")/*@res "所属单位不能为空！"*/);
    		}
        }
		...
		if(modvos.size()==0){
			throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011","UPP2011-000041")/*@res "至少要选择一个控制方式!"*/);
		}
	...
	private UIRefPane getRefdjlx() {
		if (ivjtdjlx == null) {
			...
			ivjtdjlx.getRefModel().setWherePart("(pk_billtypecode in('2631','2632') or (pk_billtypecode like '263X-%') )and billtypename is not null" );
