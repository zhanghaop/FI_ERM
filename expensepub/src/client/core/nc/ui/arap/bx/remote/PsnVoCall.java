...
//--end
	...
	public void handleResult(Map<String, Object> datas) throws BusinessException {
		...
		if(result!=null){
			if(result[0]!=null) {
				String key = PSN_PK_ + getPk_user() + getPk_group();
				WorkbenchEnvironment.getInstance().putClientCache(key, result[0]);
			}
			if(result[1]!=null) {
				String key = DEPT_PK_ + getPk_user() + getPk_group();
				WorkbenchEnvironment.getInstance().putClientCache(key, result[1]);
			}
			if(result[2]!=null) {
				String key = FIORG_PK_ + getPk_user() + getPk_group();
				WorkbenchEnvironment.getInstance().putClientCache(key, result[2]);
			}
			if(result[3]!=null) {
				String key = GROUP_PK_ + getPk_user() + getPk_group();
