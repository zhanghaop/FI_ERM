...
public class VOChecker {
	 ...
	 * 前台准备数据，处理简单的赋值类调用
	 */
	public void prepare(JKBXVO bxvo) throws BusinessException {
		...
		try {
			...
			prepareChildrenVO(childrenVO);
			prepareIfNull_Fin(bxvo);
			prepareBusItemvo(bxvo);

		} catch (ValidationException e) {
			if (!parentVO.isInit()
					&& !(parentVO.getDjzt().intValue() == BXStatusConst.DJZT_TempSaved)) {
	...
	private void chkIsMustContrast(JKBXVO bxvo) throws BusinessException {
		...
		if(BXConstans.BX_DJDL.equals(bxvo.getParentVO().getDjdl())&&bxvo.getParentVO().getDjzt()!=null&&bxvo.getParentVO().getDjzt().intValue()==BXStatusConst.DJZT_Saved){
			//报销单是否必须冲借款参数
			boolean paramIsMustContrast = false;
			try {
				paramIsMustContrast = SysInit.getParaBoolean(bxvo.getParentVO().getPk_org(),BXParamConstant.PARAM_IS_FORCE_CONTRAST).booleanValue();
...
//--end
				...
				if (hasJKD && paramIsMustContrast) {
					throw new ContrastBusinessException(ContrastBusinessExceptionType.FORCE,nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("common","UPP2011V57-000002")/* @res* "本人有未清的借款单，必须进行冲借款操作!"*/);
				}
			}
		}
	}
...
//modified by chendya
			...
			if(parentVO.getPk_org()!=null && calendar==null){
				throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011v61013_0","02011v61013-0060")/*@res "业务单元未设置会计期间"*/);
			}
//--end
			...
			accperiod.setAccperiodmonth(new AccperiodmonthVO[] { calendar
					.getMonthVO() });
			...
			parentVO
					.setKjqj(accperiod.getAccperiodmonth()[0].getAccperiodmth());// 单据会计期间
	...
	private void prepareForNullJe(JKBXVO bxvo) {
		JKBXHeaderVO parentVO = bxvo.getParentVO();
	...
	public static Map<String, List<String>> getCrossItems(JKBXVO bxvo) {
		JKBXHeaderVO parentVO = bxvo.getParentVO();
		BusiTypeVO busTypeVO = BXUtil.getBusTypeVO(parentVO.getDjlxbm(),
				parentVO.getDjdl());
	...
	private static void changeItemsToMap(List<String> busiitems, String corp,
			Map<String, List<String>> corpItems) {
	...
	public void checkkSaveBackground(JKBXVO vo) throws BusinessException {
		...
		if (!vo.getParentVO().isInit()) {
			...
			// 暂存不校验
			if (BXStatusConst.DJZT_TempSaved != vo.getParentVO().getDjzt()) {
				// 外部交换校验金额
				checkHeadItemJe(vo);
	...
	private void chkCustomerPayFreezeFlag(JKBXVO bxvo) throws ValidationException {
		...
		try {
			supfivos = qryservice.getSupFinanceVO(new String[]{hbbm}, bxvo.getParentVO().getFydwbm(), new String[]{SupFinanceVO.PAYFREEZEFLAG, SupFinanceVO.PK_SUPPLIER});
		...
		if (!(ArrayUtils.isEmpty(supfivos))) {
			for (SupFinanceVO vo : supfivos) {
				...
				if (flag != null && flag.booleanValue()) {
					throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("expensepub_0","02011002-0160")/*@res "单据录入的供应商付款已经冻结，不能进行借款报销单的录入！"*/);
				}
			}
		}
	}
	...
	private void prepareBusinessType(JKBXVO bxvo) {
		...
		if (!bxvo.getParentVO().getQcbz().booleanValue()) {
			try {
				...
				if (!StringUtils.isEmpty(headvo.getDjdl())
						&& !StringUtils.isEmpty(headvo.getDjlxmc()) 
						&& !StringUtils.isEmpty(headvo.getCreator())) {
					if (headvo.getDjdl().equals(BXConstans.BX_DJDL)) {
						pk_busitype = ipf.retBusitypeCanStart(
								BXConstans.BX_DJLXBM, headvo.getDjlxbm(),
								headvo.getPk_org(), headvo.getCreator());
					} else if (headvo.getDjdl().equals(BXConstans.JK_DJDL)) {
						pk_busitype = ipf.retBusitypeCanStart(
								BXConstans.JK_DJLXBM, headvo.getDjlxbm(),
								headvo.getPk_org(), headvo.getCreator());
					}
				...
				String msg = nc.vo.ml.NCLangRes4VoTransl.getNCLangRes()
						.getStrByID("common", "UCMD1-000053")/* @res "交易类型" */
						+ headvo.getDjlxbm()
						+ nc.vo.ml.NCLangRes4VoTransl.getNCLangRes()
								.getStrByID("expensepub_0", "02011002-0011")/*
																			 * @res
																			 * "没有找到相应的流程,请在[业务流定义]配置"
																			 */
						+ headvo.getDjlxbm()
						+ nc.vo.ml.NCLangRes4VoTransl.getNCLangRes()
								.getStrByID("expensepub_0", "02011002-0012")/*
																			 * @res
																			 * "自制流程"
																			 */;
...
//--end
	 ...
	 */
	private void getBusitype(JKBXHeaderVO parentVO) throws BusinessException,
			DAOException {

		if (parentVO.getFkyhzh() != null
				&& isInneracc(parentVO.getFkyhzh()).equals("Y")) {

			IPFConfig pFConfig = NCLocator.getInstance()
			...
			if (StringUtil.isEmpty(pk_busiflow)) {
				...
				String pk_busiflowValue = pFConfig.retBusitypeCanStart(
						billtype, trade_type, parentVO.getPk_org(), userid);
				if (parentVO.getDjdl().equals(BXConstans.BX_DJDL)) {
					if (pk_busiflowValue == null) {
						throw ExceptionHandler
								.createException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011v61013_0","02011v61013-0061")/*@res "业务流程为空，请检查业务流程配置"*/);
					}
					parentVO.setBusitype(pk_busiflowValue);

					new BaseDAO().updateVO(parentVO,
							new String[] { "busitype" });
				}
			}
		}
...
//	    UFBoolean bflage = (UFBoolean)dao.executeQuery(sql, processor)
...
//			}
	...
	private void chkCashaccountAndFkyhzh(JKBXHeaderVO headerVO) throws BusinessException{
		...
		if((!StringUtil.isEmpty(fkyhzh))&&(!StringUtil.isEmpty(pkCashaccount))){
			throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011v61_1215_0","02011v61215-0000")/*@res "单位银行帐号和现金帐户不能同时有值"*/);
		}
	}
	...
	private void chkBankAccountCurrency(JKBXVO vo) throws BusinessException{
		if(vo==null||vo.getParentVO()==null){
			throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("expensepub_0","02011002-0135")/*@res "单据为空"*/);
		}
		...
		if(vos==null||vos.length==0||!pk_currtype.equals(vos[0].getPk_currtype())){
			throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("expensepub_0","02011002-0136")/*@res "!个人银行帐号对应的币种和单据币种不一致"*/);
		}
	}
	...
	private void checkAuditMan(JKBXVO bxvo) throws BusinessException {
		...
		if (auditman == null) {
			throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl
					.getNCLangRes().getStrByID("2011", "UPP2011-000365")/*
																		 * @res
	...
	private void checkToPublicPay(JKBXVO bxvo) throws BusinessException {
		...
		if (receiver != null && hbbm != null) {
			throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("expensepub_0","02011002-0137")/*@res "供应商与收款人不能同时输入！"*/);
		}
		if (receiver == null && hbbm == null) {
			throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("expensepub_0","02011002-0138")/*@res "供应商与收款人必须输入一个！"*/);
		}
	...
	private void checkBillDate(JKBXHeaderVO parentVO) throws ValidationException {
		...
		if(parentVO.getDjrq()==null){
			//数据交换平台可能录入空的单据日期
			throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011ermpub0316_0","02011ermpub0316-0012")/*@res "单据日期不能为空"*/);
		}
		...
		try {
			...
			if (yearMonth != null && yearMonth.length() != 0) {
				...
				if(year!=null&&month!=null){
					...
					if(calendar==null){
						throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011v61013_0","02011v61013-0021")/*@res "组织的会计期间为空"*/);
					}
					calendar.set(year, month);
					if(calendar.getMonthVO()==null){
						throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011v61013_0","02011v61013-0022")/*@res "组织起始期间为空"*/);
					}
		...
		if (startDate == null) {
			ExceptionHandler.consume(new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("expensepub_0","02011002-0001")/* @res "该组织模块启用日期为空" */));
		}
		if (startDate != null) {
			...
			throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl
					.getNCLangRes().getStrByID("expensepub_0", "02011002-0141")/*
																				 * @res
																				 * "该业务单元未设置期初期间"
																				 */);
		}
	...
	private void checkCurrencyRate(JKBXHeaderVO parentVO)
			throws BusinessException {
		...
		if (hl == null || hl.toDouble() == 0) {
			throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl
					.getNCLangRes().getStrByID("2011", "UPP2011-000395")/*
																		 * @res
																		 * "汇率值不能为0！"
																		 */);
		}
		if (isGlobalmodel) {
			if (globalhl == null || globalhl.toDouble() == 0) {
				throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("expensepub_0","02011002-0142")/*@res "全局本币模式已启用，全局汇率值不能为0！"*/);
			}
		}
		if (isGroupmodel) {
			if (grouphl == null || grouphl.toDouble() == 0) {
				throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("expensepub_0","02011002-0143")/*@res "集团本币模式已启用，集团汇率值不能为0！"*/);
			}
		}

	}

	private void prepareBusItemvo(JKBXVO bxvo) throws BusinessException {
	...
	/**
	 * 校验财务页签是否有数据，若行数为零，则根据表头的“财务核报金额”添加并设置一个新行
	 *
	 * @throws Exception
	 */
	private void prepareIfNull_Fin(JKBXVO bxvo) throws BusinessException {
		JKBXHeaderVO headVO = bxvo.getParentVO();
		if (bxvo.getChildrenVO() != null && bxvo.getChildrenVO().length != 0)
			return;
		if (bxvo.getChildrenVO() != null && bxvo.getChildrenVO().length == 1) {
			String[] jeField = JKBXHeaderVO.getJeField();
			for (String je : jeField) {
				bxvo.getChildrenVO()[0].setAttributeValue(je, headVO.getAttributeValue(je));
			}
		}
		if (bxvo.getChildrenVO() != null && bxvo.getChildrenVO().length != 0){
			return;
		}
//		ErVOUtils.prepareBxvoHeaderToItem(bxvo);
	}

	private void prepareChildrenVO(BXBusItemVO[] childrenVO)
			throws DataValidateException {
		if (childrenVO == null)
			return;

		for (BXBusItemVO vo : childrenVO) {
			if(vo.getDr()==null){
	...
	private void prepareHeader(JKBXHeaderVO parentVO, BxcontrastVO[] bxcontrastVOs)
			throws BusinessException {
		if (parentVO == null)
			return;
		...
		parentVO.setSpzt(IPfRetCheckInfo.NOSTATE);
		...
		// 设置冲销完成日期默认值
		parentVO.setContrastenddate(new UFDate(
				BXConstans.DEFAULT_CONTRASTENDDATE));
		...
		if (zfdwbm != null && bzbm != null && parentVO.getYbje() != null
				&& djrq != null) {
			// 重新设置表头本币金额
			parentVO.setBbje(Currency.computeYFB(zfdwbm, Currency.Change_YBJE,
					bzbm, parentVO.getYbje(), null, null, null, bbhl, djrq)[2]);
		}
		...
		if (!parentVO.isInit()
			...
			if (djdl.equals(BXConstans.JK_DJDL)) {
				if (parentVO.getIscheck().booleanValue()) {
					...
					if (parentVO.getZpxe() == null)
						throw new ValidationException(
								nc.vo.ml.NCLangRes4VoTransl.getNCLangRes()
										.getStrByID("2011", "UPP2011-000311")/*
																			 * @res
																			 * "空白支票借款单的支票限额必须录入!"
																			 */);
					if (parentVO.getZpxe().doubleValue() <= 0)
						throw new ValidationException(
								nc.vo.ml.NCLangRes4VoTransl.getNCLangRes()
										.getStrByID("2011", "UPP2011-000312")/*
																			 * @res
																			 * "支票限额必须录入正数!"
																			 */);
				} else {
					if (parentVO.getYbje() == null)
						throw new ValidationException(
								nc.vo.ml.NCLangRes4VoTransl.getNCLangRes()
										.getStrByID("2011", "UPP2011-000313")/*
																			 * @res
																			 * "借款金额必须录入!"
																			 */);
					if (parentVO.getYbje()==null)
						throw new ValidationException(
								nc.vo.ml.NCLangRes4VoTransl.getNCLangRes()
										.getStrByID("2011", "UPP2011-000314")/*
																			 * @res
																			 * "借款金额必须录入正数!"
																			 */);
				...
				if (parentVO.getYbje() == null && parentVO.getYbje().doubleValue() < 0)
					throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl
							.getNCLangRes()
							.getStrByID("2011", "UPP2011-000315")/*
																 * @res
																 * "报销金额必须录入!"
																 */);
			}
			...
			if (bxcontrastVOs != null) {
				for (BxcontrastVO vo : bxcontrastVOs) {
					...
					// 重新设置冲借款本币金额
					vo.setCjkbbje(Currency.computeYFB(zfdwbm,
							Currency.Change_YBJE, bzbm, vo.getCjkybje(), null,
							null, null, bbhl, djrq)[2]);
					vo.setFybbje(Currency.computeYFB(zfdwbm,
							Currency.Change_YBJE, bzbm, vo.getFyybje(), null,
							null, null, bbhl, djrq)[2]);

					UFDouble[] ggcjkbbje = Currency.computeGroupGlobalAmount(vo
							.getCjkybje(), vo.getCjkbbje(), bzbm, djrq,
							parentVO.getPk_org(), parentVO.getPk_group(), globalbbhl, groupbbhl);
					UFDouble[] ggfybbje = Currency.computeGroupGlobalAmount(vo
							.getFyybje(), vo.getFybbje(), bzbm, djrq, parentVO
			...
			adjuestCjkje(parentVO, cjkybje, cjkbbje, groupcjkbbje,
					globalcjkbbje);
		}
	 ...
	 */
	private void checkValidHeader(JKBXHeaderVO parentVO)
			throws ValidationException {
		parentVO.validate();
	}
	...
	private void checkHeadFinItemJe(JKBXVO bxvo) throws ValidationException {
		...
		JKBXHeaderVO parentVO = bxvo.getParentVO();
		String[] keys = new String[] { "ybje", "bbje", "ybye", "bbye",
				"hkybje", "hkbbje", "zfybje", "zfbbje", "cjkybje", "cjkbbje" };
		String[] name = new String[] {
				nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011",
						"UPP2011-000280")/* @res "原币金额" */,
				nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011",
						"UPP2011-000245")/* @res "本币金额" */,
				nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011",
						"UPP2011-000318")/* @res "原币余额" */,
				nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011",
						"UPP2011-000246")/* @res "本币余额" */,
				nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011",
						"UPP2011-000319")/* @res "还款原币金额" */,
				nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011",
						"UPP2011-000320")/* @res "还款本币金额" */,
				nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011",
						"UPP2011-000321")/* @res "支付原币金额" */,
				nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011",
						"UPP2011-000322")/* @res "支付本币金额" */,
				nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011",
						"UPP2011-000323")/* @res "冲借款原币金额" */,
				nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011",
						"UPP2011-000324") /* @res "冲借款本币金额" */};
		int length = keys.length;
		for (int j = 0; j < length; j++) {

			UFDouble headJe = parentVO.getAttributeValue(keys[j]) == null ? new UFDouble(
					0)
					: (UFDouble) parentVO.getAttributeValue(keys[j]);
			UFDouble bodyJe = new UFDouble(0);
			for (int i = 0; i < childrenVO.length; i++) {
				UFDouble je = childrenVO[i].getAttributeValue(keys[j]) == null ? new UFDouble(
						0)
			...
			if (headJe.compareTo(bodyJe) != 0) {
				// 本币金额误差的容错处理
				if (j % 2 == 1
						&& headJe.sub(bodyJe).abs().compareTo(new UFDouble(1)) < 0) {
					parentVO.setAttributeValue(keys[j], bodyJe);
					continue;
				}
				throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl
						.getNCLangRes().getStrByID("2011", "UPP2011-000325",null,new String[]{name[j]})/*
																			 * @res
																			 * "表头"i"和财务页签金额合计不一致!"
																	 */);
			}
	...
	private void checkHeadItemJe(JKBXVO bxvo) throws ValidationException {
		...
		if (bxvo.getParentVO().getDjdl().equals(BXConstans.BX_DJDL)) {
			...
			if (total.compareTo(amount) != 0) {
				throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl
						.getNCLangRes().getStrByID("2011", "UPP2011-000327")/*
																			 * @res
			...
			if (ybje.compareTo(amount) != 0) {
				throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl
						.getNCLangRes().getStrByID("2011", "UPP2011-000328")/*
																			 * @res
	 ...
	 * @throws ValidationException
	 */
	private void checkValidChildrenVO(BXBusItemVO[] childrenVO)
			throws ValidationException {
		childrenVO = removeNullItem(childrenVO);
		
		if((childrenVO == null || childrenVO.length == 0)){
			throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011v61013_0","02011v61013-0089")/*@res "请增加财务信息!"*/);
		}

		for (BXBusItemVO child : childrenVO) {
			child.validate();
			if (child.getTablecode() == null) {
				throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("expensepub_0","02011002-0144")/*@res "表体页签信息不能为空!"*/);
			}
		}
	}
	...
	private void checkFinRange(JKBXVO bxvo, JKBXHeaderVO parentVO)
			throws ValidationException {
		if (bxvo.getParentVO().getDjdl().equals(BXConstans.BX_DJDL)) {
			// 表头财务核报金额与合计金额的容差校验
			...
			try {
				...
				if (SysInit.getParaInt(parentVO.getPk_org(),
						BXParamConstant.PARAM_ER_FI_RANGE) == null)
					throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl
							.getNCLangRes()
							.getStrByID("2011", "UPP2011-000332")/*
																 * @res
																 * "获取参数“财务核报容差范围”时出错!"
																 */);
				range = SysInit.getParaInt(parentVO.getPk_org(),
						BXParamConstant.PARAM_ER_FI_RANGE).doubleValue();
			} catch (BusinessException e) {
				throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl
						.getNCLangRes().getStrByID("2011", "UPP2011-000332")/*
																			 * @res
																			 * "获取参数“财务核报容差范围”时出错!"
																			 */);
			}
			if (range == null)
				throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl
						.getNCLangRes().getStrByID("2011", "UPP2011-000333")/*
																			 * @res
																			 * "未得到参数“财务核报容差范围”!"
																			 */);
			Double total = parentVO.getTotal() == null ? 0 : parentVO
					.getTotal().toDouble();// 合计金额
			Double ybje = parentVO.getYbje() == null ? 0 : parentVO.getYbje()
					.toDouble();// 财务核报金额

			if (range.doubleValue() < 0) { // 只能改小不能改大
				if (ybje > total) {
					throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("expensepub_0","02011002-0145")/*@res "单据总金额不能改大!"*/);
				}
			}
			if (Math.abs(total - ybje) > Math.abs(range))
				throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl
						.getNCLangRes().getStrByID("2011", "UPP2011-000334")/*
																			 * @res
	...
	private void checkValidFinItemVO(JKBXVO bxvo) throws ValidationException {
		...
		if (childrenVO != null&&childrenVO.length>0) {
			...
			if (ispay && isreceive) {
				throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl
						.getNCLangRes().getStrByID("2011", "UPP2011-000397")/*
																			 * @res
	...
	private void checkValidContrast(JKBXVO bxvo, JKBXHeaderVO parentVO)
			throws ValidationException {
		if (parentVO.getIscheck().booleanValue()
				&& parentVO.getDjdl().equals(BXConstans.BX_DJDL)) {
			if (bxvo.getContrastVO() == null
					|| bxvo.getContrastVO().length == 0) {
				throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl
						.getNCLangRes().getStrByID("2011", "UPP2011-000335")/*
																			 * @res
																			 * "保存失败,空白支票报销单必须进行冲借款操作!"
																			 */);
			}
		}
	}
	...
	public static void  checkErmIsCloseAcc(JKBXVO bxvo) throws BusinessException {
		...
		//非期初单据才校验
		if(bxvo.getParentVO().getQcbz()==null||
				!bxvo.getParentVO().getQcbz().booleanValue()){
			checkIsCloseAcc(moduleCode, pk_org, date);
		}
	}

	/**
	 * 批量根据模块号，业务单元oid,会计期间(年-月)判断是否关账。若已关帐抛出异常
	 *
	 * @param moduleCode
	 * @param pk_org
	 * @param date
	 * @throws BusinessException
	 */
	private static void checkIsCloseAcc(String moduleCode, String pk_org,
		...
		Map<String, Boolean> res = NCLocator.getInstance().lookup(
				ICloseAccQryPubServicer.class).isCloseByModuleIdAndPk_org(
				moduleCode, pk_org, new String[] { period });
		if (res.get(period)!=null
				&&res.get(period).booleanValue()) {
			msg = nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("expensepub_0","02011002-0146")/*@res "已经关帐，不能进行该操作！"*/;
		}
