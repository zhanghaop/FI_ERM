	...
	public void doTransferArap(JKBXVO vo) throws BusinessException {
		...
		try {
			...
			if(head.getZfybje().compareTo(new UFDouble(0))!=0){
				if(!head.getPk_org().equals(head.getFydwbm())){
					pubTransInfo(map,new TransFerInfo(head.getPk_org_v(),head.getPk_org(),head.getFydwbm(),head.getZfybje(),head.getZfbbje(),head.getGlobalzfbbje(),head.getGroupzfbbje(),vo,1));
					pubTransInfo(map,new TransFerInfo(head.getFydwbm_v(),head.getFydwbm(),head.getPk_org(),head.getZfybje(),head.getZfbbje(),head.getGlobalzfbbje(),head.getGroupzfbbje(),vo,2));
				}
			}
			...
			if(head.getCjkybje().sub(head.getHkybje()).compareTo(new UFDouble(0))!=0){
				...
				for(BxcontrastVO constrst:constrasts){
					String jkDwbm = constrst.getPk_org();
					if(!jkDwbm.equals(head.getFydwbm())){
						...
						if(ybje.compareTo(new UFDouble(0))!=0){
							pubTransInfo(map,new TransFerInfo(null,jkDwbm,head.getFydwbm(),ybje,bbje,globalbbje,groupbbje,vo,1));
							pubTransInfo(map,new TransFerInfo(head.getFydwbm_v(),head.getFydwbm(),jkDwbm,ybje,bbje,globalbbje,groupbbje, vo,2));
						}
					}
				}
			}
