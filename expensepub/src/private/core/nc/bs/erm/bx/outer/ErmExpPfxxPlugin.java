...
//					return bxvos[0].getParentVO().getDjbh();
					...
					Object object = pfbo.processAction("SAVE",header.getDjlxbm(),null , bxvo, null,null );
					return bxvo.getParentVO().getDjbh();
...
//		noteVO.setPk_businesstype(IPFConfigInfo.STATEBUSINESSTYPE);//FIXME
	...
	private void setDeptOrgVersion(final JKBXHeaderVO header) throws BusinessException {
		...
		String[] orgs = new String[]{header.getPk_org(), header.getDwbm(), header.getFydwbm(), header.getPk_pcorg()};
