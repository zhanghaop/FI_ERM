...
import nc.impl.er.djlx.ArapBillTypeImpl;
...
public class ErForCmpBO implements ISettleNotifyPayTypeBusiBillService {
	...
	public void billStateChange(BusiInfo busiInfo, BusiStateTrans trans)
		...
		// 审核->签字
		if (BusiStatus.Audit.equals(trans.getFrom())
				&& BusiStatus.Sign.equals(trans.getTo())) {
			jkbxVO = getBxVO(busiInfo, trans);
			bo.settle(busiInfo.getOperator(), busiInfo.getOperatorDate(),jkbxVO);

		}
		// 签字->反签字
		else if (BusiStatus.Sign.equals(trans.getFrom())
				&& BusiStatus.Audit.equals(trans.getTo())) {
			jkbxVO = getBxVO(busiInfo, trans);
	...
	public AggregatedValueObject getBillVO(BusiInfo info)
			throws BusinessException {
		return getBxVO(info);
	}

	private AggregatedValueObject getBxVO(BusiInfo info)
			throws BusinessException {
		return getBxVO(info, null);
	}

	public boolean isAutoFillEbankInfo(String pk_corp, String pk_billtype)
			throws BusinessException {
		return false;
	}
	...
	private Integer getPayStatus(NetPayExecInfo payInfo)
		...
		if (payInfo.getExecStatusMap() == null) {
			throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl
					.getNCLangRes().getStrByID("2011v61013_0",
							"02011v61013-0027")/* @res "回写的网银支付状态不明" */);
		}
		nc.vo.cmp.CMPExecStatus[] status = payInfo.getExecStatusMap().values()
				.toArray(new CMPExecStatus[0]);
		...
		for (int i = 0; i < status.length; i++) {
			if (status[i] == nc.vo.cmp.CMPExecStatus.SomePayFinish) {
				...
				isFinished = true;
				if (isFailed) {
					retStatus = nc.vo.cmp.CMPExecStatus.SomePayFinish
							.getStatus();
					break;
				}
	...
	public void netPayExecChange(NetPayExecInfo payInfo)
		...
		try {
			...
			// 这里对自定义交易类型进行下处理
			if (djlxbm.startsWith(BXConstans.BX_DJLXBM)
					|| djlxbm.startsWith(BXConstans.JK_DJLXBM)) {
				head.setDjdl(new DjLXDMO().getDjlxvoByDjlxbm(djlxbm,
						BXConstans.GLOBAL_CODE).getDjdl());
			} else {
				head.setDjdl(new DjLXDMO().getDjlxvoByDjlxbm(djlxbm,
						BXConstans.GROUP_CODE).getDjdl());
			}
			((BXSuperDAO) new JKBXDAO()).update(new JKBXHeaderVO[] { head },
					new String[] { JKBXHeaderVO.PAYFLAG, JKBXHeaderVO.PAYDATE,
							JKBXHeaderVO.PAYMAN, JKBXHeaderVO.JSR, JKBXHeaderVO.JSRQ,
							JKBXHeaderVO.JSH });
		} catch (Exception e) {
	 ...
	 *             调用结算接口
	 */
	public void invokeCmp(JKBXVO param, UFDate date, BusiStatus busiStatus)
			throws BusinessException {
		JKBXHeaderVO head = param.getParentVO();
		if (SettleUtil.hasSettleInfo(head, head.getDjdl())) {
			ISettlement settle = (ISettlement) NCLocator.getInstance().lookup(ISettlement.class.getName());
			DjLXVO djlxvo = new ArapBillTypeImpl().getDjlxvoByDjlxbm(param.getParentVO().getDjlxbm(), param.getParentVO().getPk_group());
			...
			if (busiStatus == busiStatus.Save) {
				...
				settle.notifySettlementBatchDelete(SettleUtil.getCmpMsg(param,
						djlxvo, date, busiStatus));
			} else {
	...
	public void notify4HandSettle(List<String> idList, boolean isOpp,
		...
		for (String id : idList) {
			List<JKBXHeaderVO> name = new BXZbBO().queryHeadersByPrimaryKeys(
					new String[] { id }, BXConstans.BX_DJDL);
			if (name == null || name.size() == 0) {
				name = new BXZbBO().queryHeadersByPrimaryKeys(new String[] { id }, BXConstans.JK_DJDL);
			}
			JKBXHeaderVO head = name.get(0);
			head.setPk_jkbx(id);
			head.setPayflag(isOpp ? CMPExecStatus.UNPayed.getStatus()
					: CMPExecStatus.PayFinish.getStatus());
			head.setPaydate(isOpp ? null : operateDate);
			head.setPayman(isOpp ? null : operator);
			voList.add(head);
		}
		new BaseDAO().updateVOArray(voList.toArray(new JKBXHeaderVO[] {}),
				new String[] { JKBXHeaderVO.PAYFLAG, JKBXHeaderVO.PAYDATE,JKBXHeaderVO.PAYMAN });
	}
	...
	public List<SettlementBodyVO> autoBX(List<SettlementBodyVO> bodyList)
		...
		for (SettlementBodyVO vo : bodyList) {
			if (PfDataCache.getBillType(vo.getPk_billtype())
					.getParentbilltype().equals(BXConstans.BX_DJLXBM)) {
				result.add(vo);
			}
		}
		return result;
	...
	public List<String> getFromNoticeBill(List<String> idList)
			throws BusinessException {
		
		return null;
	}

	public boolean isAutoFillEbankInfo(String arg0, String arg1, String arg2)
			throws BusinessException {
		return false;
	}
	...
	private void notifyErmCommitToFts(ECommitFtsType type , BusiInfo... busiInfos) throws BusinessException{
		...
		List<JKBXHeaderVO> vos = bo.queryHeadersByPrimaryKeys((String[])pk_billList.toArray(new String[0]),getDJDL(busiInfos[0]));
		for(JKBXHeaderVO vo : vos){
			vo.setPayflag(ECommitFtsType.CANCEL_COMMIT.equals(type) ? CMPExecStatus.UNPayed.getStatus() : CMPExecStatus.Paying.getStatus());
		}
		//更新支付状态
		bo.updateHeaders((JKBXHeaderVO[])vos.toArray(new JKBXHeaderVO[0]), new String[]{JKBXHeaderVO.PAYFLAG});
	}
