...
public class ErInitDAtaBO extends AbstractUpdateAccount implements IInitData {// nc.bs.sm.install.IUpdateData{
	...
	private void updateSysTemplate(String oldVersion, String newVersion) throws DAOException {
		if(is60Update261(oldVersion, newVersion)){
			...
			//处理pub_systemplate表中的Pk_corp和templateflag字段
			//1.先查出所有
			sql="select pk_group from org_group";
			List org_group = (List)getBaseDAO().executeQuery(sql, new ColumnListProcessor());
			
			//2.查出所有的SystemplateVO
			sql="(funnode like '20110ETEA%' or funnode like '20110ETLB%') and funnode not in ('20110ETEA','20110ETLB')  and pk_corp = '@@@@' and tempstyle in ('1','3')";
			Collection<SystemplateVO> needCopyVo = getBaseDAO().retrieveByClause(SystemplateVO.class, sql);
			
			//3.Templateflag为true
			if(needCopyVo != null && needCopyVo.size() > 0){
				List<SystemplateVO> arrList= new ArrayList();
				for(int i=0;i<org_group.size();i++){
					for (Iterator<SystemplateVO> iterator = needCopyVo.iterator(); iterator.hasNext();) {
						SystemplateVO systemplateVO = (SystemplateVO) iterator.next().clone();
						systemplateVO.setPk_corp((String)org_group.get(i));
						systemplateVO.setTemplateflag(UFBoolean.TRUE);
						arrList.add(systemplateVO);
					}
				}
				getBaseDAO().insertVOArray(arrList.toArray(new SystemplateVO[0]));
			}
		}
	...
	public void doAfterUpdateData(String oldVersion, String newVersion) throws Exception {
		...
		//同步修改模板分配中自定义查询模板的id
		updateSysTempletForQueryTemplet();
		...
		//11.升级后处理移动模板
		updateModTemplet(newVersion);
		
		
	}
	
	private void updateModTemplet(String newVersion) throws DAOException{
		//1.先查出所有自定义类
		String sql="select distinct funnode from pub_systemplate where "
		+"(funnode like '20110ETEA%' or funnode like '20110ETLB%') "+
		"and funnode not in ('20110ETEA','20110ETLB') ";
		List def = (List)getBaseDAO().executeQuery(sql, new ColumnListProcessor());
		
		//2.查出所有默认的移动模板
		sql="funnode  in ('20110ETEA','20110ETLB') and tempstyle in ('5')";
		Collection<SystemplateVO> needCopyVo = getBaseDAO().retrieveByClause(SystemplateVO.class, sql);
		
		//3.Templateflag为Funnode
		if(needCopyVo != null && needCopyVo.size() > 0){
			List<SystemplateVO> arrList= new ArrayList();
			for(int i=0;i<def.size();i++){
				for (Iterator<SystemplateVO> iterator = needCopyVo.iterator(); iterator.hasNext();) {
					SystemplateVO systemplateVO = (SystemplateVO) iterator.next().clone();
					if(((String)def.get(i)).startsWith("20110ETEA")){
						if(systemplateVO.getFunnode().equals("20110ETEA")){
							systemplateVO.setFunnode((String)def.get(i));
							arrList.add(systemplateVO);
						}
						else{
							continue;
					...
					if(((String)def.get(i)).startsWith("20110ETLB")){
						if(systemplateVO.getFunnode().equals("20110ETLB")){
							systemplateVO.setFunnode((String)def.get(i));
							arrList.add(systemplateVO);
						}
						else{
	...
	private void updateSysTempletForQueryTemplet() throws BusinessException {
		Logger.debug("*************************************");
		Logger.debug("******** 报销管理模块升级6.0升级到6.1更新模板分配中查询模板的id开始"+getClass().getName()+"**********");
		Logger.debug("*************************************");
		...
		//需要替换的查询模板的id
		sql = "(funnode like '20110ETEA%' or funnode like '20110ETLB%') and funnode not in ('20110ETEA','20110ETLB')  and pk_corp <> '@@@@' and tempstyle in ('1')";
		Collection<SystemplateVO> needReplaceVo = getBaseDAO().retrieveByClause(SystemplateVO.class, sql);
	
		if(needReplaceVo != null && needReplaceVo.size() > 0){
			for (Iterator<SystemplateVO> iterator = needReplaceVo.iterator(); iterator.hasNext();) {
				SystemplateVO systemplateVO = (SystemplateVO) iterator.next();
				String funcode = systemplateVO.getFunnode();
				...
				if( temp!= null && temp.contains("借款")){/*-=notranslate=-*/
					systemplateVO.setTemplateid(jkQueryTempletId);
				}else if(temp!= null && temp.contains("报销")){/*-=notranslate=-*/
					systemplateVO.setTemplateid(bxQueryTempletId);
				}else {
					systemplateVO.setTemplateid(bxQueryTempletId);
				}
			}
			getBaseDAO().updateVOArray(needReplaceVo.toArray(new SystemplateVO[0]));
		}
		Logger.debug("*************************************");
		Logger.debug("******** 报销管理模块升级6.0升级到6.1更新模板分配中查询模板的id开始"+getClass().getName()+"**********");
		Logger.debug("*************************************");
	...
	private void updateQueryTemplet() throws BusinessException {
		...
		if(needReplaceVo != null && needReplaceVo.size() > 0){
			for (Iterator iterator = needReplaceVo.iterator(); iterator.hasNext();) {
	...
	private void updateInitBillVersion(String newVersion) throws Exception {
		...
		if (newVersion != null && newVersion.startsWith("6.1")) {
			...
			sql="update pub_billtemplet_b  set  showflag='1', listshowflag='Y', editflag='1' where pk_billtemplet in " +
