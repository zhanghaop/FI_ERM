...
public class ErmReleaseQryObjSelectComp extends UIPanel {
	...
	// 查询对象选择表列名
	private List<String> colNameList = Arrays.asList(new String[] {nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("common","UC000-0002770")/*@res "查询对象"*/, nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("feesaccount_0","02011001-0040")/*@res "查询对象的次序"*/});
...
//		initialize();
	...
	private void initOther() {
		...
		UILabel accountFormatLabel = new UILabel(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("feesaccount_0","02011001-0041")/*@res "帐页格式"*/);
		...
		reportFormatCombo.addItems(new DefaultConstEnum[] {
				new DefaultConstEnum(IPubReportConstants.ACCOUNT_FORMAT_LOCAL, FipubReportResource.getAccountFormatLocalLbl()),
				new DefaultConstEnum(IPubReportConstants.ACCOUNT_FORMAT_FOREIGN, FipubReportResource.getAccountFormatForeignLbl()) });
...
//		multiUnitSepChk.doClick();
	...
	public List<Object> getSelectQryObj() {
		...
		for (int i = 0; i < qryObjTblRowCount; i++) {
			Object qryObj = ((PubUIComboBox)((DefaultCellEditor)qryObjTable.getCellEditor(i, 0)).getComponent()).getSelectdItem();
			if (qryObj == null) {
				continue;
			}
	...
	public void getUseableQryObj() {
		try {
			...
			qryObjVOList = NCLocator.getInstance().lookup(IReportQueryObjRegQuery.class)
					.getRegisteredQueryObj(IErmReportConstants.ERM_PRODUCT_CODE_Lower);

			// 设置可选查询对象的值
			setQryObjTableData();
		} catch (BusinessException e) {
			MessageDialog.showErrorDlg(this, nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("feesaccount_0","02011001-0005")/*@res "错误"*/, nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("feesaccount_0","02011001-0042")/*@res "获取查询对象时发生错误！"*/);
		}
	}
